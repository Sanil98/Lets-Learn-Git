...
# Specify the tables to be backed up for each database
# Format: {'database_name': ['table1', 'table2', ...]}
tables_to_backup = {
    "pradan_hr": ["table1", "table2"],
    "pradan_grow": ["table3", "table4"],
    # Add tables for other databases as needed
}

for db, tables in tables_to_backup.items():
    print(db)
    suffix = "/" + db + "-" + today + ".sql"
    print(suffix)
    
    for table in tables:
        dumpcmd = f"mysqldump -u {DB_USER} -p{DB_USER_PASSWORD} -h {DB_HOST} {db} {table} > {BACKUP_PATH}{suffix}"
        print(f"Creating Table-backup for {db}.{table}")
        ...
        try:
            ...
            if result == 0:
                ...
                msg['To'] = ", ".join(["***@pradan.net", "***@pardan.net"])  # Use valid email addresses
                body = f"DB BACKUP FAILED FOR DB: {db}"
                body = MIMEText(body, "plain")
                msg.attach(body)
                print(msg)

                mail.sendmail("serverforpradan@pradan.net", ["***@pradan.net", "***@pradan.net"], msg.as_string())  # Use valid email addresses
                mail.quit()
            # (Rest of the script remains unchanged)
    ...
    # Continue with the rest of the code for backing up entire databases
    print(db)
    suffix = "/" + db + "-" + today + ".sql"
    print(suffix)
